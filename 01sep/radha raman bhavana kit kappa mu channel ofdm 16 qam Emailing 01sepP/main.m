...
for d=1:dmax
    ...
    for snr1=0:2:50
        ...
        if kmuuse==1
            kappa=.1;
            mu=.1;
%             [chdata,fadingratio]=kmunew(ofdmsignal');
%             [chdata,fadingratio]=kmunew(ofdmsignal',kappa);
            [chdata1,fadingratio]=kmunew(ofdmsignal',kappa,mu);
            preamp=inv(fadingratio)-1;% assumed adaptive pre amplifier relative to fading
%             preamp=25;
            chdata=preamp.*chdata1;
        end    
